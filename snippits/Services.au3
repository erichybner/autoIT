...
;~ 	$tServiceMain = DllCallbackRegister("_Main", "none", "dword;ptr")
	...
	DllCall($hAdvapi32_DLL, "int", "StartServiceCtrlDispatcherW", "ptr", DllStructGetPtr($tdispatchTable))
...
;~ 	_Main()
	...
	Local $ret = DllCall($hAdvapi32_DLL, "ptr", "RegisterServiceCtrlHandler", "ptr", DllStructGetPtr($tServiceName), "ptr", DllCallbackGetPtr($tServiceCtrl)) ;register service
	If $ret[0] = 0 Then
...
Func _Service_SetServiceStatus($hServiceStatus, $lpServiceStatus)
	Local $ret = DllCall($hAdvapi32_DLL, "int", "SetServiceStatus", "ptr", $hServiceStatus, "ptr", $lpServiceStatus)
