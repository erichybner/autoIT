...
Global Const $IEAU3VersionInfo[6] = ["T", 3, 0, 0, "20120903", "T3.0-0"]
...
Func _IENavigate(ByRef $o_object, $s_Url, $f_wait = 1)
	...
	If @error Then ; Trap COM error, report and return
		Local $s_error = @error, $s_function_name = "_IENavigate"
		__IEErrorNotify("Error", $s_function_name, "$_IEStatus_COMError", $s_error)
		Return SetError($_IEStatus_ComError, $s_error, 0)
	EndIf
	If $f_wait Then
	...
	Select
		Case __IEIsObjType($o_object, "browser"); InternetExplorer
			While Not (String($o_object.readyState) = "complete" Or $o_object.readyState = 4 Or $f_Abort)
				...
				If @error Then
					$s_error = @error
					If __IEComErrorUnrecoverable($s_error) Then
						$i_ErrorStatusCode = __IEComErrorUnrecoverable($s_error)
						$f_Abort = True
					EndIf
				EndIf
	...
	Switch $i_ErrorStatusCode
		...
		Case $_IEStatus_AccessIsDenied
			__IEErrorNotify("Warning", "_IELoadWait", "$_IEStatus_AccessIsDenied", _
					"Cannot verify readyState.  Likely casue: cross-domain scripting security restriction. (" & $s_error & ")")
		...
		Case $_IEStatus_ClientDisconnected
			__IEErrorNotify("Error", "_IELoadWait", "$_IEStatus_ClientDisconnected", _
					$s_error & ", Browser has been deleted prior to operation.")
...
Func _IEIsFrameSet(ByRef $o_object)
	...
	Else
		If @error Then ; Trap COM error, report and return
			Local $s_error = @error, $s_function_name = "_IEIsFrameSet"
			__IEErrorNotify("Error", $s_function_name, "$_IEStatus_COMError", $s_error)
			Return SetError($_IEStatus_ComError, $s_error, 0)
		EndIf
	...
	For $link In $links
		...
		If $linktext = $s_linkText Then
			If ($found = $i_index) Then
				$link.click()
				If @error Then ; Trap COM error, report and return
					Local $s_error = @error, $s_function_name = "_IELinkClickByText"
					__IEErrorNotify("Error", $s_function_name, "$_IEStatus_COMError", $s_error)
					Return SetError($_IEStatus_ComError, $s_error, 0)
				EndIf
				If $f_wait Then
...
Func _IELinkClickByIndex(ByRef $o_object, $i_index, $f_wait = 1)
	...
	If ($i_index >= 0) And ($i_index <= $oLinks.length - 1) Then
		...
		$oLink.click()
		If @error Then ; Trap COM error, report and return
			Local $s_error = @error, $s_function_name = "_IELinkClickByIndex"
			__IEErrorNotify("Error", $s_function_name, "$_IEStatus_COMError", $s_error)
			Return SetError($_IEStatus_ComError, $s_error, 0)
		EndIf
		If $f_wait Then
	...
	For $img In $imgs
		...
		If StringInStr($linktext, $s_linkText) Then
			If ($found = $i_index) Then
				$img.click()
				If @error Then ; Trap COM error, report and return
					Local $s_error = @error, $s_function_name = "_IEImgClick"
					__IEErrorNotify("Error", $s_function_name, "$_IEStatus_COMError", $s_error)
					Return SetError($_IEStatus_ComError, $s_error, 0)
				EndIf
				If $f_wait Then
...
Func _IEFormElementGetValue(ByRef $o_object)
	...
	If @error Then ; Trap COM error, report and return
		Local $s_error = @error, $s_function_name = "_IEFormElementGetValue"
		__IEErrorNotify("Error", $s_function_name, "$_IEStatus_COMError", $s_error)
		Return SetError($_IEStatus_ComError, $s_error, 0)
	EndIf
...
Func _IEFormElementSetValue(ByRef $o_object, $s_newvalue, $f_fireEvent = 1)
	...
	If @error Then ; Trap COM error, report and return
		Local $s_error = @error, $s_function_name = "_IEFormElementSetValue"
		__IEErrorNotify("Error", $s_function_name, "$_IEStatus_COMError", $s_error)
		Return SetError($_IEStatus_ComError, $s_error, 0)
	EndIf
	If $f_fireEvent Then
		...
		If String($img.type) = "image" Then
			...
			If StringInStr($linktext, $s_linkText) Then
				If ($found = $i_index) Then
					$img.click()
					If @error Then ; Trap COM error, report and return
						Local $s_error = @error, $s_function_name = "_IEFormImageClick"
						__IEErrorNotify("Error", $s_function_name, "$_IEStatus_COMError", $s_error)
						Return SetError($_IEStatus_ComError, $s_error, 0)
					EndIf
					If $f_wait Then
...
Func _IEFormSubmit(ByRef $o_object, $f_wait = 1)
	...
	$o_object.submit()
	If @error Then ; Trap COM error, report and return
		Local $s_error = @error, $s_function_name = "_IEFormSubmit"
		__IEErrorNotify("Error", $s_function_name, "$_IEStatus_COMError", $s_error)
		Return SetError($_IEStatus_ComError, $s_error, 0)
	EndIf
	If $f_wait Then
...
Func _IEFormReset(ByRef $o_object)
	...
	$o_object.reset()
	If @error Then ; Trap COM error, report and return
		Local $s_error = @error, $s_function_name = "_IEFormReset"
		__IEErrorNotify("Error", $s_function_name, "$_IEStatus_COMError", $s_error)
		Return SetError($_IEStatus_ComError, $s_error, 0)
	EndIf
	Return SetError($_IEStatus_Success, 0, 1)
...
Func _IETableWriteToArray(ByRef $o_object, $f_transpose = False)
	...
	For $tr In $trs
		...
		For $td In $tds
			...
			If @error Then ; Trap COM error, report and return
				Local $s_error = @error, $s_function_name = "_IETableWriteToArray"
				__IEErrorNotify("Error", $s_function_name, "$_IEStatus_COMError", $s_error)
				Return SetError($_IEStatus_ComError, $s_error, 0)
			EndIf
			$col = $col + $td.colSpan
...
Func _IEBodyWriteHTML(ByRef $o_object, $s_html)
	...
	If @error Then ; Trap COM error, report and return
		Local $s_error = @error, $s_function_name = "_IEBodyWriteHTML"
		__IEErrorNotify("Error", $s_function_name, "$_IEStatus_COMError", $s_error)
		Return SetError($_IEStatus_ComError, $s_error, 0)
	EndIf
...
Func _IEDocWriteHTML(ByRef $o_object, $s_html)
	...
	Local $oTemp = $o_object.document
	If @error Then ; Trap COM error, report and return
		Local $s_error = @error, $s_function_name = "_IEDocWriteHTML"
		__IEErrorNotify("Error", $s_function_name, "$_IEStatus_COMError", $s_error)
		Return SetError($_IEStatus_ComError, $s_error, 0)
	EndIf
	_IELoadWait($oTemp)
...
Func _IEDocInsertText(ByRef $o_object, $s_string, $s_where = "beforeend")
	...
	If @error Then ; Trap COM error, report and return
		Local $s_error = @error, $s_function_name = "_IEDocInsertText"
		__IEErrorNotify("Error", $s_function_name, "$_IEStatus_COMError", $s_error)
		Return SetError($_IEStatus_ComError, $s_error, 0)
	EndIf
	Return SetError($_IEStatus_Success, 0, 1)
...
Func _IEDocInsertHTML(ByRef $o_object, $s_string, $s_where = "beforeend")
	...
	If @error Then ; Trap COM error, report and return
		Local $s_error = @error, $s_function_name = "_IEDocInsertHTML"
		__IEErrorNotify("Error", $s_function_name, "$_IEStatus_COMError", $s_error)
		Return SetError($_IEStatus_ComError, $s_error, 0)
	EndIf
	Return SetError($_IEStatus_Success, 0, 1)
...
Func _IEHeadInsertEventScript(ByRef $o_object, $s_htmlFor, $s_event, $s_script)
	...
	If @error Then ; Trap COM error, report and return
		Local $s_error = @error, $s_function_name = "_IEHeadInsertEventScript"
		__IEErrorNotify("Error", $s_function_name, "$_IEStatus_COMError", $s_error)
		Return SetError($_IEStatus_ComError, $s_error, 0)
	EndIf
	Return SetError($_IEStatus_Success, 0, 1)
...
Func _IEAction(ByRef $o_object, $s_action)
	...
	EndIf
	;
	Local $s_error = @error, $s_function_name = "_IEAction"
...
Func _IEPropertySet(ByRef $o_object, $s_property, $newvalue)
	...
	If @error Then ; Trap COM error, report and return
		Local $s_error = @error, $s_function_name = "_IEPropertySet"
		__IEErrorNotify("Error", $s_function_name, "$_IEStatus_COMError", $s_error)
		Return SetError($_IEStatus_ComError, $s_error, 0)
	EndIf
	Return SetError($_IEStatus_Success, 0, 0)
...
Func _IEQuit(ByRef $o_object)
	...
	$o_object.quit()
	If @error Then ; Trap COM error, report and return
		Local $s_error = @error, $s_function_name = "_IEQuit"
		__IEErrorNotify("Error", $s_function_name, "$_IEStatus_COMError", $s_error)
		Return SetError($_IEStatus_ComError, $s_error, 0)
	EndIf
	$o_object = 0
...
; Author ........: Dale Hohm
; Modified ......: maybe no more needed  (jpm)
; ===============================================================================================================================
Func __IEInternalErrorHandler()
	$IEComErrorScriptline = $o__IEErrorHandler.scriptline
	$IEComErrorNumber = $o__IEErrorHandler.number
	$IEComErrorNumberHex = Hex($o__IEErrorHandler.number, 8)
	$IEComErrorDescription = StringStripWS($o__IEErrorHandler.description, 2)
	$IEComErrorWinDescription = StringStripWS($o__IEErrorHandler.WinDescription, 2)
	$IEComErrorSource = $o__IEErrorHandler.Source
	$IEComErrorHelpFile = $o__IEErrorHandler.HelpFile
	$IEComErrorHelpContext = $o__IEErrorHandler.HelpContext
	$IEComErrorLastDllError = $o__IEErrorHandler.LastDllError
	$IEComErrorOutput = ""
...
; Author ........: Dale Hohm
; ===============================================================================================================================
Func __IEComErrorUnrecoverable($s_error)
	Select
		; Cross-domain scripting security error
		Case ($s_error = -2147352567) ; "Access is denied."
			Return $_IEStatus_AccessIsDenied
			;
			; Browser object is destroyed before we try to operate upon it
		Case ($s_error = -2147417848) ; "The object invoked has disconnected from its clients."
			Return $_IEStatus_ClientDisconnected
			;
			; Browser object is destroyed before we try to operate upon it
		Case ($s_error = -2147023179) ; "The interface is unknown."
			Return $_IEStatus_ClientDisconnected
			;
		Case Else
